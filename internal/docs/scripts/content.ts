...
export const rewriteHref = (
  ...
  // Either published parent: `./packages/core` and `./tools/mcp` both point at an
  // API page, and both are spelled that way in the repo's own markdown.
  const workspace = /^(?:packages|tools)\//.exec(base);
  if (workspace) {
    const pkg = targets.packages.get(
      base.slice(workspace[0].length).replace(/\/$/, ''),
    );
    if (pkg) return pkg;
  }
 ...
 * published in `internal/docs/README.md` so it is predictable without reading this
 * file.
 */
