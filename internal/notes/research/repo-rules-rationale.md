# Repo rules: the rationale record

This is `CLAUDE.md` as it stood at 1,045 lines, before it was cut to an index.

It is kept because the cut removed restatement, not reasoning: every measurement
here was real, and several paragraphs record an alternative that was tried and
reversed. `CLAUDE.md` now states the rule and names the gate that enforces it;
when you need to know _why_ a rule exists, or what was rejected on the way to it,
it is here.

`internal/` is exempt from `scripts/no-slop.test.ts`, so this file keeps the
...
Everything in it exists because two frontends had written it twice. Both consumers
inline what they import, so anything added is paid for twice; read
...
`internal/docs` builds with **Vite**, having been reversed from `Bun.build` by
re-measuring. Re-measure before reversing it again: the numbers are in
[architecture/tooling.md](../../../docs/architecture/tooling.md), "Documentation site", and
only there.

`internal/dashboard-ui` **does** use Vite: its bundle is inlined byte-for-byte into
a page a backend serves, so tree-shaking earns its cost. **It is where all the React
and Mantine live** - `packages/dashboard` contains none and cannot, being published
as plain ESM that must not oblige a consumer to install React or run a bundler to
serve a page. Its build writes a `src/ui-bundle.ts` into the package: generated,
committed, and regenerated by that package's own `build` so it cannot go stale. Read
its README before touching it; every component added costs bytes twice, in JS and in
the CSS list in `src/styles.ts`.
...
All nine are uniform. `@dunx/openapi` has an extra term, since it wraps the app
root: `[options.root, ...(options.imports ?? [])]`.

**Test it with a `token()`, never a class.** An unbound class self-binds into
whichever scope asks first, so a class resolves whether or not `imports` reached the
factory and the test passes against the bug it guards.
...
- **`lint:check` is `oxlint --max-warnings 0`.** A warning fails it, the same as an
  error. `typescript/await-thenable` is `off` for test files, because `bun-types`
...
together. **`unit` is on request**, because `coverage` runs the same files: running
both at once had them competing for the same Redis. `bun run ci <phase>` runs one
while iterating, `bun run ci --list` names them. Step output is captured and printed
only on failure, so a green run is one line per step.

The `browser` phase is `internal/docs/browser/`: the built site in a real Chrome
through **`Bun.WebView`**, asserting what happy-dom cannot. No playwright, and no
browser download. **It is also where the screenshots come from** - each test writes
its frame before it asserts, and `bun run shots` is that suite plus a build, so
there is no second traversal to drift from it. Two traps it has to work around are
...
- **`render()` has no auto-cleanup under `bun test`**, so `internal/docs`'s preload
  registers `afterEach(cleanup)` for every file. Do not drop that line, and do not
  put a `document.body.innerHTML = ''` in its place: clearing the DOM does not
  unmount, and the leak cost that suite 4.5x its runtime.
...
- **The floor assumes the backing services are reachable, and the `coverage` job is
  the one place that declares them**: valkey, postgres, and MinIO through a
...
- `S3_ENDPOINT` is **not** exported, even though `Bun.S3Client` reads it for every
  client that sets none: it turned the offline presign suite into assertions about
  MinIO. The live block takes `DUNX_S3_TEST_ENDPOINT` explicitly.
- **Test scaffolding is counted as shipped code unless excluded.**
  ...
  and the seeder directories the db suites write into the temp dir. Scaffolds alone
  held `@dunx/openapi` five points under its real figure.
...
- **Do not try to correct the denominator for those**, and do not raise the floor
  without reading that section: the rule behind the biggest artifact does not add up
  at scale, so a correction built on it would be wrong in a way harder to notice
  than the artifact. `@dunx/core` is the binding constraint and is at its measurable
  ceiling, not under-tested.
