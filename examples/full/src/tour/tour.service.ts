...
import { RpcDemo } from '../rpc/rpc.demo.js';
...
export class Tour {
  ...
  async run(app: HttpApp, url: string): Promise<void> {
    ...
    this.group(
      '@dunx/http/connect - protobuf over Connect and gRPC-Web, same port',
    );
    await this.rpc.demonstrate(url);
