 ...
 *
 * Injectable — `WsModule.forRoot()` binds it, so a service can publish without
 * holding a socket.
 */
export class PubSub {
  #server: Server<SocketData> | undefined;

  /** Called with the live server by the adapter; also usable directly. */
    ...
    if (!this.#server) {
      throw new AppError(
        'PubSub has no server yet. Publish once the server is listening — ' +
          'WsFactory.listen() attaches it, as does the first upgrade.',
      );
    }
    return this.#server;
  }
