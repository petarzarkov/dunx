...
import { inject, Module, type DynamicModule } from '@dunx/core';
import {
  Gateway,
  OnClose,
  OnMessage,
...
import { WsModule } from '../module.js';
import { WsFactory, type WsApp } from './factory.js';
...
const rootModule = (options: DynamicModule): DynamicModule => ({
  module: WsModule,
  imports: [options],
});
...
describe('a gateway on a real server', () => {
  ...
  beforeAll(async () => {
    @Module({
      imports: [
        WsModule.forRoot({
          port: 0,
          gateways: [ChatGateway, AdminGateway],
          idleTimeout: 8,
    ...
    app = await WsFactory.create(AppModule);
  ...
  it('serves every declared gateway path', () => {
    expect([...app.adapter.paths].sort()).toEqual(['/admin', '/chat']);
  });
  ...
  it('answers a path no gateway serves with 404', async () => {
    ...
    expect(
      app.adapter.handles(new Request(new URL('/nothing', url).href)),
    ).toBe(false);
  });
});
...
describe('WsFactory', () => {
  it('resolves a gateway through the container, constructor deps and all', async () => {
    ...
    const app = await WsFactory.create(
      rootModule(WsModule.forRoot({ port: 0, gateways: [GreetGateway] })),
    );
  ...
  it('takes options from forRootAsync', async () => {
    @Gateway('/late')
    class LateGateway {
      @OnMessage()
      raw(message: string | Buffer): string {
        return `late:${String(message)}`;
      }
    }

    const app = await WsFactory.create(
      rootModule(
        WsModule.forRootAsync({
          gateways: [LateGateway],
          useFactory: async () => {
            await Bun.sleep(1);
            return { port: 0, maxPayloadLength: 1024 };
          },
        }),
      ),
    );
    ...
    try {
      const client = await connect(url, '/late');
      client.socket.send('now');
      expect(await client.next()).toBe('late:now');
      client.close();
    } finally {
      await app.shutdown();
    }
  ...
  it('says what to import when no options are bound', async () => {
    @Module({})
    class Bare {}

    expect(await settled(WsFactory.create(Bare))).toContain(
      'No WebSocket options are bound',
    );
  });
  ...
  it('reports a gateway with no handlers at boot', async () => {
    ...
    expect(
      await settled(
        WsFactory.create(
          rootModule(WsModule.forRoot({ port: 0, gateways: [EmptyGateway] })),
        ),
      ),
    ).toContain('EmptyGateway is registered as a gateway');
  });
});
