...
import { HandlerKind } from '../gateway/marker.js';
  ...
  if (gateway.handlers.length === 0) {
    throw new AppError(
      `${gateway.name} is registered as a gateway but declares no handlers. ` +
        'Add an @OnMessage/@OnOpen/... method, or move it to providers.',
    );
  }
...
/**
 * Bun dispatches an upgrade by path, so two gateways on one path would mean one of
 * them could never receive a connection. That is a boot error naming both.
 */
export const buildGateways = (
  ...
  if (byPath.size === 0) {
    throw new AppError(
      'No gateways were registered. Declare them with ' +
        'WsModule.forRoot({ gateways: [ChatGateway] }).',
    );
  }
