...
- **`dunx_start`** - the runtime, the scaffold command, the three commands that add
  dunx to a project that already exists, an index of the guide, and four rules. The
...
- **`dunx_guide`** - the 23 chapters of `docs/guide`. No arguments returns the index;
  `search` returns matching lines with chapter and line number, capped at 40; `topic`
...
writes `tools/mcp/src/generated.ts`, committed and regenerated by that package's own
`build`. Same arrangement as `packages/dashboard/src/ui-bundle.ts`, and 439 KB
against its 445 KB.
