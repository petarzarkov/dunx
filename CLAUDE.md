...
`internal/ui` is `@dunx/ui`: the shared Mantine theme, consumed **as source** by
`internal/docs` and `internal/openapi-ui`, with no build step - its `exports` point at
`src/`, and each consumer's bundler compiles what it imports. Keep it small. The
explorer bundle is inlined byte-for-byte into `@dunx/openapi`, so anything added there
is paid for twice; the two things already tried and rejected are recorded in its
`src/index.ts`.
...
`internal/openapi-ui` **does** use Vite, for the opposite reason: its bundle is
...
cannot go stale. Read `internal/openapi-ui/README.md` before touching it; every
...
`openapi-ui` (the explorer bundle), `ui` (the shared theme).

Nine published workspaces, deliberately few. Merging is nearly free because the runtime weight is
