...
internal/<name>/        # Private workspaces, never published - docs, bench, openapi-ui, ui
...
- **Never declare a union another package already declares.** `methodColor` and
  ...
  it imports the constant now.
- **A wire format is declared once, by the server, and the frontend imports it by
  relative `import type`.** `internal/openapi-ui` and `internal/dashboard-ui` both
  read their payload types out of `packages/*/src`, so the page cannot drift from
  the handler that fills it and there is no build-order edge.
...
`internal/openapi-ui` and `internal/dashboard-ui` **do** use Vite, for the opposite
reason: each bundle is inlined byte-for-byte into a page a backend serves, so
Rollup's tree-shaking earns its ~1.5 s. **These two are where all the React and
Mantine live** - `packages/openapi` and `packages/dashboard` contain none and cannot,
being published as plain ESM that must not oblige a consumer to install React or run
a bundler to serve a page. Each build writes a `src/ui-bundle.ts` into its package:
generated, committed, and regenerated by that package's own `build` so it cannot go
stale. Read their READMEs before touching them; every component added costs bytes
twice, in JS and in the CSS list in `src/styles.ts`.

Both bundles sit behind a **`./ui`** subpath and are reached with `await import()` -
from `OpenApiExplorer.page()` and from `DashboardMiddleware`'s first page request - so
importing either package does not load one. Each `html.ts` takes the script to inline
as an argument (`renderShell`) and **must not import `ui-bundle.ts`**, or the split
silently reverts.
...
| `@dunx/openapi`   | OpenAPI 3.1 from the routes' own zod schemas; `internal/openapi-ui`'s explorer inlined behind `./ui` (zod is a `peerDependency`)                                                                      |
...
`openapi-ui` (the explorer bundle), `dashboard-ui` (the dashboard page), `ui` (the
