...
| Realtime           | WebSocket gateways and `@Sse`, with a Redis or Postgres relay for many nodes  |
...
| Caching            | Memory, Redis or the two tiered, with single flight on a miss                 |
| Background work    | bullmq over `Bun.RedisClient`, sandboxed processors, `@Cron`                  |
| Messaging          | RabbitMQ topic exchanges, one handler per queue, the trace carried across     |
...
| Failing well       | `ResiliencePolicy` around anything: timeout, retry, backoff, jitter, fallback |
| Operating it       | Health checks, structured logging, request and driver metrics, a dashboard    |
...
- **[The live demo](https://demo.dunx.win)** - `examples/full` running on a
  Raspberry Pi 5: the API explorer, the ops dashboard, bull-board, and a
  walkthrough that drives twenty-five capabilities from a browser, including a
  transaction rolling back, a CSRF refusal, an idempotent retry, an order
  crossing a RabbitMQ topic exchange, and the process's own request, query and
  event-loop timings
...
| [`@dunx/mcp`](./tools/mcp) | [![npm](https://img.shields.io/npm/v/%40dunx%2Fmcp)](https://www.npmjs.com/package/%40dunx%2Fmcp) [![dls](https://img.shields.io/npm/dt/%40dunx%2Fmcp?label=dls)](https://www.npmjs.com/package/%40dunx%2Fmcp) [![size](https://img.shields.io/npm/unpacked-size/%40dunx%2Fmcp?label=size)](https://www.npmjs.com/package/%40dunx%2Fmcp) | [![cov](https://dunx.win/badges/coverage-mcp.svg)](https://dunx.win/coverage) | A Model Context Protocol server for dunx: the written guide and scaffold catalogue with no app, plus your routes, providers and modules once you point it at one |
